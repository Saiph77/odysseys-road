...
const manifest: AssetManifest = { assets: [{
  id: 'fixture', path: '/fixture', frameCount: 100, fps: 24, width: 1280, height: 720,
  clips: { sample: { from: 11, to: 91 } },
}] };
const scene: SceneFrame = {
  sceneId: 'fixture', renderer: 'sequence', asset: 'fixture', clip: 'sample',
  localProgress: 0, opacity: 1, layer: 10,
};

describe('plannedFrameIndex', () => {
  it.each([[0, 11], [0.5, 51], [1, 91], [-1, 11], [2, 91]])('maps progress %s to clip frame %s', (progress, expected) => {
    expect(plannedFrameIndex({ ...scene, localProgress: progress }, manifest)).toBe(expected);
  });

  it('applies the configured micro-scrub range', () => {
    expect(plannedFrameIndex({ ...scene, behavior: { scrub: { from: 0.2, to: 0.8 } } }, manifest)).toBe(27);
    expect(plannedFrameIndex({ ...scene, localProgress: 1, behavior: { scrub: { from: 0.2, to: 0.8 } } }, manifest)).toBe(75);
  });
