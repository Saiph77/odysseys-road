...
test('forty physical screens, chapter navigation and real scroll agree', async ({ page }) => {
  ...
  await expect
    .poll(async () => Number(await page.locator('.dev-road-panel dd').nth(1).textContent()))
    .toBeGreaterThan(90);
  ...
  for (const [progress, scene] of [
    ...
    expect(
      await panel
        .locator('p')
        .evaluateAll((lines) =>
          lines.every((line) => line.getBoundingClientRect().bottom <= innerHeight * 0.88),
        ),
    ).toBe(true);
