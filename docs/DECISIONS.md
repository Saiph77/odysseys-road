...
| D-003 | 摄像头生命周期 | 页面加载即请求，全程运行到页面关闭，不随章节停止 |
| D-004 | Pre-roll | 无按钮：加载即请求摄像头并滚动校准；第一次滚动即开始正片（V2，2026-09-05 用户拍板） |
...
- 摄像头在页面加载完成后立即请求（D-004 V2），成功后**一直运行到页面关闭**；不在 Selected 后停止，也不在章节切换时停止。
...
## D-004 Pre-roll：无按钮，第一次滚动即开始（V2）

> V1（单按钮「开始归航」）已被用户于 2026-09-05 否决："直接采用无按钮的设计，首次直接就开始。"

页面加载后停在剧本 01 §2 描述的"活画"（黎明前海岸 + 标题 + 副标题），**没有任何按钮**：

1. `load` 完成后立即 `getUserMedia()`；标题下方一行极小字随状态变化：
   `海正在辨认你的方向…` → 就绪 `以头部航行` / 失败或 4s 未就绪 `以指针航行`。文字不阻塞，任何时候都可以滚动。
2. **滚动校准**：摄像头就绪后持续维护最近 36 帧（≈1.5s）的滚动窗口；第一次滚动发生时取该窗口的脸宽中值 → z 基线、focusX 均值 → 中心偏置。窗口内 `detected` 帧 < 12 时使用默认基线（z=0，偏置 0），不等待。
3. **开始信号**：第一次 `wheel`（deltaY>0）、`ArrowDown` / `PageDown` / `Space` 或指针向下拖动 → 60 秒计时开始，`troy` 章从 Road 0 起推进。首次滚动本身的 delta 计入进度，不吞掉。
4. 活画底部保留两行极小字：`方向仅在本机即时计算，不保存影像。` 与 `本体验包含一次瞬间强光。`（见 D-007）。

音频（D-009 第一版无声）：`wheel` 不构成浏览器"用户激活"，不能解锁 `AudioContext`。将来加音频时，在**选择窗口的 click/Enter** 或首次 `keydown` 处 `resume()`；`troy` 章在此之前保持无声，由字幕承担信息。不为音频再加按钮。

`?demo=track3`：跳过摄像头请求，直接指针模式，其余流程相同。
