...
function validateSceneRoad(scene: SceneDefinition, chapter: ChapterDefinition, issues: ValidationIssue[]) {
  ...
  if (!Number.isFinite(scene.blend.in) || !Number.isFinite(scene.blend.out) || scene.blend.in < 0 || scene.blend.out < 0) {
...
function validateClip(
  ...
  if (!scene.asset) {
    if (scene.clip) issues.push(issue('CLIP_WITHOUT_ASSET', `Scene "${scene.id}" has clip but no asset`, `scenes.${scene.id}`));
    return;
  }
...
function validateTransition(scene: SceneDefinition, manifest: AssetManifest, issues: ValidationIssue[]) {
  const treatment = scene.behavior?.treatment;
  if (treatment && !['memory', 'zoom'].includes(treatment.kind)) {
    issues.push(issue('TREATMENT_KIND', `Unknown treatment kind "${treatment.kind}"`, `scenes.${scene.id}`));
  }
  ...
  for (const side of ['from', 'to'] as const) {
    ...
    if (!ref || !['first', 'last'].includes(ref.frame)) {
      issues.push(issue('TRANSITION_REF', `Invalid transition ${side} frame reference`, `scenes.${scene.id}`));
      if (!ref) continue;
    }
...
function validateSequenceOverlap(chapter: ChapterDefinition, issues: ValidationIssue[]) {
  ...
  for (let index = 0; index < sequences.length; index += 1) {
    ...
    for (const previous of sequences.slice(0, index)) {
      const transitionLayer = current.layer === 20 && current.behavior?.transition && previous.behavior?.transition;
      if (current.layer !== previous.layer || transitionLayer) continue;
      if (current.road.start >= previous.road.end || previous.road.start >= current.road.end) continue;
      issues.push(
        issue(
          'SEQUENCE_OVERLAP',
...
function validateChapterTimeline(chapter: ChapterDefinition, issues: ValidationIssue[]) {
  if (!Number.isFinite(chapter.scroll.screens) || chapter.scroll.screens <= 0) {
    issues.push(issue('SCREENS_INVALID', `Chapter "${chapter.id}" screens must be positive and finite`));
  }
  if (!Number.isFinite(chapter.timeline.end) || chapter.timeline.end <= chapter.timeline.start) {
    issues.push(issue('TIMELINE_INVALID', `Chapter "${chapter.id}" timeline must have a finite positive span`));
  }
  ...
  if (chapter.kind === 'hub') {
    ...
    } else if (!Number.isFinite(chapter.gate.atRoad) || chapter.gate.atRoad < chapter.timeline.start || chapter.gate.atRoad > chapter.timeline.end) {
      issues.push(issue('GATE_RANGE', `Hub "${chapter.id}" gate is outside its timeline`));
    }
  }
  let coveredUntil = chapter.timeline.start;
  for (const scene of [...chapter.scenes].sort((first, second) => first.road.start - second.road.start)) {
    if (scene.road.start > coveredUntil) {
      issues.push(issue('TIMELINE_GAP', `Chapter "${chapter.id}" has uncovered Road before ${scene.road.start}`));
    }
...
export function validateStory(
  ...
  if (config.release.defaultTrackId != null && !Object.hasOwn(tracks, config.release.defaultTrackId)) {
