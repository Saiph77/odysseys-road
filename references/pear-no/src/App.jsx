...
import SpatialWork from './spatial/SpatialWork';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Browsers can restore scrollY before React mounts. Sync once immediately
    // so the road timeline, chapter rail and lazy spatial scene match it.
    handleScroll();
  ...
  useEffect(() => {
    // Prewarm shortly before The Work (0.224–0.336) without adding a hidden
    // Three.js context to the critical hero scene.
    if (scrollProgress > 0.16) setSpatialMounted(true);
  }, [scrollProgress]);
  ...
  return (
    <main id="top" className="spatial-integrated" aria-busy={!bootLoaded && !bootError}>
      <LoadingState
        ready={bootLoaded}
        error={bootError}
        phase={resourcePhase}
      ...
      <div className="stage">
        <div className="pin">
          ...
          {/* Real HTML surfaces in a head-coupled Three.js room. Wheel input
              remains passive so Pear's original road timeline is untouched. */}
          {spatialMounted && <SpatialWork scrollProgress={scrollProgress} />}
